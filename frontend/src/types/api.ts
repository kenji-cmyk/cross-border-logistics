...
export const orderSchema = z.object({ orderId: z.string(), customerId: z.string(), quotationId: z.string(), deliveryAddress: z.string(), totalAmountVnd: z.number(), depositAmountVnd: z.number(), remainingAmountVnd: z.number(), status: orderStatusSchema, ownerToken: z.string().optional(), remainingPaidAt: z.string().optional(), createdAt: z.string(), updatedAt: z.string(), items: z.array(orderItemSchema) });
...
export const refundSchema=z.object({refundId:z.string(),paymentId:z.string(),orderId:z.string(),amountVnd:z.number(),status:paymentStatusSchema});
export const financialSummarySchema=z.object({orderId:z.string(),payments:z.array(paymentSchema).nullish().transform(v=>v??[]),refunds:z.array(refundSchema).nullish().transform(v=>v??[]),paidVnd:z.number(),refundedVnd:z.number(),netPaidVnd:z.number()});
